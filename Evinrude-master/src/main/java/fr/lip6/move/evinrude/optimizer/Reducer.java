			...
			for (IReduction reduc : reducs.keySet()) {
				...
				if (ret) {
					if (type == IReduction.FLAT) {
						...
						CamiExport.dumpcamiRepresentation(model, "round" + cpt + "_reduc" + cpt2++, executable.getFolder());
					} else {
						DotExport.dumpDotRepresentation(model, "pre_round" + cpt + "_reduc" + cpt2++, executable.getFolder());
						CamiExport.dumpcamiRepresentation(model, "pre_round" + cpt + "_reduc" + cpt2++, executable.getFolder());
					}
				}
			}
