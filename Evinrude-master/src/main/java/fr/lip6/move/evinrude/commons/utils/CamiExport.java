...
public final class CamiExport {
        ...
        private static Logger LOG = Logger.getLogger(CamiExport.class.getName());
         ...
         * 
         * Exports the sub-module selected by CAMI orders
         *param Model the sub-module convert
         *return A set of commands CAMI
         */
        private String toCami(ISubModel model) {
          ...
          //      cami.append("DB()\n");
                ...
                for (ITransition transition : model.getTransitions()) {
                    ...
                    /*    if (!(transition.getEventGuard().toString()).equals("")) {
                                cami.append("CT(5:guard,").append(id).append(",").append(this.makeString(transition.getEventGuard().toString())).append(")\n");
                        }*/
                }
                ...
                for (IArc arc : model.getArcs()) {
                        ...
                        if (!arc.getValuation().equals("1")) {
                                cami.append("CT(9:valuation,").append(id).append(",").append(this.makeString(arc.getValuation().toString())).append(")\n");
                        }
                }
    	...
    	public static void dumpcamiRepresentation(IModel model, String id, File outputDirectory) {
    		try {
    			String filename = model.getHead().getName() + "_" + id + ".cami";
    			PrintWriter camiExport = new PrintWriter(new BufferedWriter(new FileWriter(outputDirectory.getAbsolutePath() + "/" + filename)));
    			CamiExport e=new CamiExport();
    			camiExport.print(e.toCami(model));
    			camiExport.close();
    			LOG.config("\t   The CAMI representation :: " + id + " :: is available in: " + filename);
    		} catch (IOException e) {
