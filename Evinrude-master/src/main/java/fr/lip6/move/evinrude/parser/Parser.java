...
public class Parser implements IParser {
	 ...
	 */
	/**dropoff window
	* Loading CFG source of an application
	*param Name name of the application
	*param Iterator files CFG files
	*return IApplication gathering all the information contained in the CFG
	*/
	public final IApplication parse(String name, Iterator<IFile> files) {
		...
		//// Create the application that will contain all the information
		...
		// Each source produces a CFG file associated to the application
		...
		while (files.hasNext()) {
			...
			/*Adds a CFG in the application
The CFG is created by the application

parameters:
cfg The CFG to add to the list of CFG*/
		...
		try {
			...
			LOG.warning("Error XML backup application....");
			System.err.println("Erreur lors de la sauvegarde XML de l'application " + e.getMessage());
			System.err.println("Error XML backup application " + e.getMessage());
			e.printStackTrace();
		}
		LOG.info("reached here" +application.getName() + ".xml");
	 ...
	 */
	/*
	 * * Launching Evinrude all benchs
*param ParamApps List of applications bench depot to process, if the list is empty all applications are processed.
*param Negate if <code> true </ code> paramApps will be treated as a list of application not to treat
*return List of applications found on the bench Depot.
	 */
	 ...
	 * / **dropoff window
* Extraction of basic statistics on the application that will be analyzed
*param Application Application considered
* /dropoff window
	 */
	 ...
	 */
	/* Analysis of a file
	*param Application The application depends so the CFG
	*param File The file to be analyzed
	*return The CFG product or <code> null </ code> if the operation went wrong
	**/
	public final ICfg extractCFG(IApplication application, IFile file) {
		/*
Open Declaration fr.lip6.move.evinrude.commons.model.cfg.Cfg.Cfg (IApplication application, String fileName, IdGenerator idGen)
...
parameters:
Application The application to which the CFG
fileName The file name which is extracted CFG
idGen id generator  
