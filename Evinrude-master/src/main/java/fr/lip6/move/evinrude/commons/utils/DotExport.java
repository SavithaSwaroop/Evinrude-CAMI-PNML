...
public final class DotExport {
	 ...
	 */
	/**
	 * Exports the model size DOT
	 *param Model The model to be exported
	 *return A string describing the format of the DOT model
	 * @param model
	 * @return
	 */
	private static String toDot(ISubModel model) {
		...
		/* If the model is the highest level (or a nested sub-model */
		...
		for (IPlace place : model.getPlaces()) {
			...
			// Graphics processing different if instead is a reference
	...
	/**
	 * Extracts a DOT representation of the current model and writes to a file
	 *param Id The identifier of the current step of execution for the model treaty
	 *param Model The model treaty ( to dump in DOT)
